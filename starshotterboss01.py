...
class StarSoldier:
    ...
    def update(self):
        ...
        if self.scene == SCENE_TITLE:
            ...
            elif self.scene != SCENE_PAUSE: # elifを使うことでポーズ直後のupdate実行を阻止
                self.update_play()
    ...
    def init_boss(self):
        ...
        m_hp = hp_table.get(self.stage, 100 + self.stage * 50)
    ...
    def update_boss(self):
        ...
        # --- ステージごとの難易度係数を設定 ---
        # 低いステージほど速度を遅く、発狂(Panic)タイミングを遅らせる
        difficulty_speed = 0.5 + (self.stage * 0.1) # STG1:0.6, STG5:1.0
        panic_threshold = 0.15 + (self.stage * 0.05) # 低いステージほどギリギリまで発狂しない
        ...
        # --- ボスの移動パターンの調整 ---
        descent_speed = 0.15 * 0.6 # 全体的に降下をよりゆっくりに
        if self.stage == 1:
        ...
        if b["hp"] <= 0: 
            self.score += 10000; self.scene = SCENE_STAGE_CLEAR; self.clear_timer = 0; self.warp_effect = 0; self.boss = None; pyxel.play(2, 2); pyxel.stop()
