...
class StarSoldier:
    ...
    def update(self):
        ...
        # --- タイトル画面の処理 ---
        if self.scene == SCENE_TITLE:
            # 【修正箇所】タイトル画面に入った際、フラグが立っていない状態（コマンド入力前）にする
    ...
    def handle_start_button(self):
        if self.scene in (SCENE_TITLE, SCENE_TUTORIAL):
            ...
            self.cheat_enabled = cheat_on # reset_allで消えるため再セット
            if self.cheat_enabled:
                self.power = POWER_MAX
                self.barrier_hp = 3
    ...
    def draw(self):
        ...
        bg_themes = {
            ...
            4: (0, 5, "warp"),     # 4面: 超空間（加速線）
            5: (0, 1, "fortress")   # 5面: 巨大要塞内部（近未来的スクロール）
        }
        curr_stg = self.stage if self.scene not in (SCENE_TITLE, SCENE_TUTORIAL) else 1
