...
class StarSoldier:
        ...
        if self.scene == SCENE_TITLE:
            ...
            if current_key:
                if current_key == self.command_list[self.command_index]:
                    ...
                    self.command_index = 0 # 間違えたらリセット
    ...
    def handle_start_button(self):
        if self.scene in (SCENE_TITLE, SCENE_TUTORIAL):
            ...
            # チート有効時は初期ステータスを強化
            if self.cheat_enabled:
                stg = self.stage # 現在選択されているステージを保持
                self.reset_all()
                self.stage = stg
    ...
    def update_boss(self):
        ...
        # --- BGMの切り替え（テンポアップ演出） ---
        if hp_rate < 0.3 and not b["panic"]:
            b["panic"] = True
            pyxel.playm(4, loop=True) # 速いテンポのBGM(Music 4)に切り替え
            
        if hp_rate < 0.3: 
            speed_mult = 1.5; density_mult = 0.6
        elif hp_rate < 0.6: 
            speed_mult = 1.2; density_mult = 0.8
        ...
        if self.scene != SCENE_GAMEOVER:
            ...
            if pyxel.frame_count % shot_interval == 0:
                ...
                if self.stage >= 2 and random.random() < (0.2 + self.stage * 0.15):
                    ...
                    self.enemy_shots.append([b["x"], b["y"], 0, (2.2 + (self.stage * 0.1)) * 0.6 * speed_mult])
                
                if self.stage >= 3 and pyxel.frame_count % (shot_interval * 3) == 0:
                    for a in [-45, -20, 20, 45]:
                        rad = math.radians(90 + a)
                        self.enemy_shots.append([b["x"], b["y"], math.cos(rad)*2.2*0.6*speed_mult, math.sin(rad)*2.2*0.6*speed_mult])
    ...
    def draw(self):
        ...
        for e in self.enemies:
            ...
            if e["type"] == "sotta":
                ...
                if self.stage == 2:
                    ...
                    pyxel.pset(ex-4, ey-4, 12); pyxel.pset(ex+4, ey-4, 12)
                    pyxel.pset(ex-3, ey-1, 7); pyxel.pset(ex+3, ey-1, 7)
                else:
                    pyxel.rect(ex-4, ey-3, 9, 7, 4); pyxel.rect(ex-2, ey-4, 5, 9, 2)
        ...
        if self.boss and (self.scene != SCENE_TITLE and self.scene != SCENE_TUTORIAL):
            ...
            if hp_rate < 0.3: b_base, b_core = 8, 2
            elif hp_rate < 0.6: b_base, b_core = 2, 8
            else: b_base, b_core = 2, 1

            if self.boss["dmg_t"] > 0 and pyxel.frame_count % 2 == 0:
                pyxel.rect(bx-20, by-16, 40, 32, 7)
            else:
                pyxel.rect(bx-14, by-12, 28, 24, 1); pyxel.rectb(bx-14, by-12, 28, 24, 5)
                pyxel.circ(bx, by, 7, b_base); core_col = 8 if pyxel.frame_count % 10 < 5 else 10
                if hp_rate < 0.3: core_col = 7 if pyxel.frame_count % 4 < 2 else 8
                ...
                if self.stage == 2:
                    pyxel.rectb(bx-21, by-17, 42, 34, 7)
        ...
        if self.scene == SCENE_TITLE:
            ...
            # --- チート有効時のメッセージ表示 ---
            if self.cheat_enabled:
                pyxel.text(20, 92, f"SPECIAL MODE: STAGE {self.stage} SELECTABLE", 11)
        elif self.scene == SCENE_TUTORIAL:
