...
class StarSoldier:
    ...
    def draw(self):
        ...
        if theme[2] == "nebula":
            ...
            # 5面: 要塞内部の高速降下（近未来的スクロール）
            pyxel.cls(1) # 少し明るい紺色をベースに
            ...
            for i in range(2):
                ...
                if pyxel.frame_count % 30 < 15:
                    pyxel.rect(30, y_base + 30, 10, 5, 8) # 赤
                    pyxel.rect(120, y_base + 30, 10, 5, 8)
                else:
                    pyxel.rect(30, y_base + 40, 10, 5, 10) # 黄色
            ...
            for i in range(15):
                ...
                # 画面中央付近には描画しない（自機を目立たせるため）
                point_x = (i * 11) % W
                if point_x < 60 or point_x > 100:
                    pyxel.pset(point_x, point_y, 7)

        # 既存の流れる星（space_stars）を描画（5面は要塞内部なので描画しない）
        if theme[2] != "fortress":
