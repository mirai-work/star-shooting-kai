...
class StarSoldier:
    ...
    def reset_all(self):
        ...
        self.scene_timer = 0
        self.used_cheats = 0 
        self.muteki_enabled = False # リセット
        self.cheat_enabled = False  # --- 追加：面セレクト状態をリセット ---
    ...
    def update(self):
            ...
            if key_pressed == self.command_list[self.command_index]:
                ...
                if self.command_index >= len(self.command_list):
                    ...
                    if self.used_cheats < limit:
                        ...
                        self.used_cheats += 1
                        cheat_just_activated = True # チート成立
                    self.command_index = 0
            else:
        ...
        # チートが成立した瞬間は、そのキー入力を「操作」として使わせない
    ...
    def handle_start_button(self):
        if self.scene in (SCENE_TITLE, SCENE_TUTORIAL):
            ...
            m_on = self.muteki_enabled # 状態を引き継ぐ
            self.reset_all()
    ...
    def update_play(self):
        self.stage_timer += 1
        # 無敵チート中はタイマーを90（点滅状態）に固定する
        if self.muteki_enabled:
            self.inv_timer = 90
    ...
    def hit_player(self):
        if self.muteki_enabled: return # チート中は被弾処理をスキップ
    ...
    def init_boss(self):
        ...
        self.boss = {
            "x": 80, "y": -20, "hp": m_hp, "max_hp": m_hp, "t": 0, "dmg_t": 0, "panic": False
        }
    ...
    def draw_ending(self):
        ...
        if scroll_end_y < -20:
            ...
            if scroll_end_y < -170:
                ...
                self.power = 0
                self.barrier_hp = 0
                self.muteki_enabled = False # チート状態も解除
                pyxel.playm(0, loop=True)
