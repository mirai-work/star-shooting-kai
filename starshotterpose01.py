...
class StarSoldier:
    ...
    def init_sound(self):
        ...
        # --- ミュージック登録 (Ch0:メロディ, Ch1:ベース, Ch2:ドラム) ---
        pyxel.musics[0].set([10], [11], [13], []) # タイトル
        pyxel.musics[1].set([10], [11], [13], []) # Stage 1 (宇宙)
        pyxel.musics[2].set([30], [31], [13], []) # ボス
        pyxel.musics[3].set([40], [], [], [])    # ゲームオーバー
        pyxel.musics[4].set([32], [33], [13], []) # ボスパニック
        ...
        if self.scene in (SCENE_TITLE, SCENE_PAUSE, SCENE_GAMEOVER, SCENE_BOSS):
            ...
            if key_pressed:
                if key_pressed == self.command_list[self.command_index]:
                    ...
                    # 間違えたら最初から（ただし、今のキーが最初のキーならインデックスを1にする）
                    if key_pressed == self.command_list[0]:
                        self.command_index = 1
                    else:
                        self.command_index = 0
    ...
    def handle_start_button(self):
        if self.scene in (SCENE_TITLE, SCENE_TUTORIAL):
            pyxel.play(2, 2)
            selected_stg = self.stage
            cheat_on = self.cheat_enabled
            self.reset_all()
            self.stage = selected_stg
            self.cheat_enabled = cheat_on 
            if self.cheat_enabled:
                self.power = POWER_MAX
                self.barrier_hp = 3
            self.scene = SCENE_PLAY; pyxel.playm(1, loop=True)
        elif self.scene == SCENE_PLAY: self.scene = SCENE_PAUSE; pyxel.stop(); pyxel.play(3, 3)
        elif self.scene == SCENE_BOSS: self.scene = SCENE_PAUSE; pyxel.stop(); pyxel.play(3, 3) 
    ...
    def draw(self):
        ...
        if theme[2] == "nebula":
            ...
            for i in range(12):
                lx = (i * 15 + 7) % W
                ly_base = (pyxel.frame_count * 8 + i * 40) % (H + 40)
                pyxel.line(lx, ly_base - 20, lx, ly_base, theme[1])
        elif theme[2] == "fortress":
            pyxel.cls(1) 
            for i in range(2):
                y_base = (pyxel.frame_count * 4 + i * H) % (H * 2) - H
                pyxel.rectb(20, y_base, 30, H - 20, 13)
                pyxel.rectb(110, y_base, 30, H - 20, 13)
            ...
            pyxel.tri(W-50, band_offset - 60, W-50, band_offset, W, band_offset, 2)
            for i in range(3):
                str_y = (pyxel.frame_count * 6 + i * 60) % (H + 60) - 60
                pyxel.rect(0, str_y, 10, 40, 0)
                pyxel.rect(W-10, str_y, 10, 40, 0)
                pyxel.line(10, str_y, 10, str_y + 40, 13)
                pyxel.line(W-11, str_y, W-11, str_y + 40, 13)
            for i in range(15):
                point_y = (pyxel.frame_count * 12 + i * 13) % H
                point_x = (i * 11) % W
                if point_x < 60 or point_x > 100:
