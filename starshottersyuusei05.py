...
class StarSoldier:
    ...
    def update(self):
                ...
                if key_pressed == self.command_list[self.command_index]:
                    ...
                    if self.command_index >= len(self.command_list):
                        ...
                        if self.scene == SCENE_TITLE:
                            ...
                            # ステージごとの制限回数を定義
                            limits = {1: 0, 2: 1, 3: 2, 4: 3, 5: 5}
                            max_cheats = limits.get(self.stage, 0)
                            
                            if self.used_cheats < max_cheats:
                                pyxel.play(3, 2)
                                self.power = POWER_MAX
                                self.barrier_hp = 3
                                self.used_cheats += 1
                            else:
                                # 回数制限を超えている場合は失敗音などを鳴らす（任意）
                                pyxel.play(3, 0) 
                        # --- ここまで修正 ---
                        self.command_index = 0
                else:
        ...
        if self.scene == SCENE_TITLE:
            ...
            if self.clear_timer > 80:
                self.warp_effect += 0.2
    # 🔥ワープ強化
            if self.warp_effect > 0:
                self.warp_effect += 0.08
    ...
    def update_play_logic(self):
        ...
        # --- 修正点1: ボス戦(SCENE_BOSS)とゲームオーバー(SCENE_GAMEOVER)ではザコ敵を出さない ---
    ...
    def update_entities(self):
        ...
        for s in self.enemy_shots[:]:
            ...
            if not (-20 < s[1] < H+20 and -20 < s[0] < W+20): 
                if s in self.enemy_shots: # まだリストに存在するか確認
    ...
    def update_boss(self):
        ...
        b["t"] += 0.05 * 0.6 * speed_mult
        
        if self.stage == 1:
            b["y"] = min(25, b["y"] + 0.5 * 0.6)
            b["x"] = 80 + math.sin(b["t"] * 0.8) * 50
        elif self.stage == 2:
            # 2面ボスの動きをマイルドに修正
            b["y"] = min(25, b["y"] + 0.5 * 0.6) + math.cos(b["t"] * 1.0) * 5
            b["x"] = 80 + math.sin(b["t"] * 0.7) * 40
        elif self.stage == 3:
            b["y"] = min(25, b["y"] + 0.5 * 0.6) + abs(math.cos(b["t"] * 1.5)) * 15
            b["x"] = 80 + math.sin(b["t"] * 1.2) * 60
        elif self.stage == 4:
            b["y"] = min(25, b["y"] + 0.5 * 0.6)
            target_x = 80 + math.sin(b["t"] * 0.5) * 60
            b["x"] += (target_x - b["x"]) * 0.1
        else:
            b["y"] = min(40, b["y"] + 0.3 * 0.6)
            b["x"] = 80 + math.sin(b["t"] * 0.4) * 30

        # ボスの弾幕をステージごとに個性化
        if self.scene != SCENE_GAMEOVER:
            shot_interval = max(3, int((25 - self.stage * 4) / 0.6 * density_mult))
        ...
        if b["hp"] <= 0: 
            self.score += 10000; self.scene = SCENE_STAGE_CLEAR; self.clear_timer = 0; self.warp_effect = 1; self.boss = None; pyxel.play(2, 2); pyxel.stop()
    ...
    def next_stage(self):
        ...
        if self.stage >= MAX_STAGE: 
            self.scene = SCENE_ENDING; self.scroll_y = 0; pyxel.playm(0, loop=True)
        else: 
    ...
    def draw(self):
        ...
        if theme[2] != "fortress":
            for s in self.space_stars:
                ...
                if s[1] > 120:
                    s[1] = 0
                    s[0] = random.randint(0, 159)

                # 描画
                if self.scene == SCENE_STAGE_CLEAR:
                    if self.clear_timer % 6 < 3:
